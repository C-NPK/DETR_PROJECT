...
def train(model, train_dataloader, val_dataloader, epochs=100):
    model.train()
    ...
    np.save('model0_training_losses.npy', np.array(training_losses))
    np.save('model0_validation_losses.npy', np.array(validation_losses))
...
model.to('cpu')
torch.save(model.state_dict(), 'last_model0.pth')